...
func getIndex(w http.ResponseWriter, r *http.Request) {
	...
	err := db.Select(&results, "SELECT `id`, `user_id`, `body`, `mime`, `created_at` FROM `posts` ORDER BY `id` DESC limit 20")
	if err != nil {
		fmt.Println(err)
		return
	}
...
func getAccountName(c web.C, w http.ResponseWriter, r *http.Request) {
	...
	rerr := db.Select(&results, "SELECT `id`, `user_id`, `body`, `mime`, `created_at` FROM `posts` WHERE `user_id` = ? ORDER BY `id` DESC", user.ID)
	if rerr != nil {
		fmt.Println(rerr)
		return
	}
...
func getPosts(w http.ResponseWriter, r *http.Request) {
	...
	rerr := db.Select(&results, "SELECT `id`, `user_id`, `body`, `mime`, `created_at` FROM `posts` WHERE `created_at` <= ? ORDER BY `id` DESC", t.Format(ISO8601_FORMAT))
	if rerr != nil {
		fmt.Println(rerr)
		return
	}
